...
This directory contains integration tests to exercise Ali end-to-end to verify behavior
...
> [!IMPORTANT]
> Before running the tests locally without the "test existing" setting, compile the top-level binary from the source tree using `go build .` in addition to GPU support with cmake if applicable on your platform.  The integration tests expect to find the compiled binary at the top of the tree.
