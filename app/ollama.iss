; Inno Setup Installer for Ali
;
; If the Visual C++ runtime is missing, the installer will download it from Microsoft during setup.
...
#define MyAppURL "https://ali.com/"
#define MyAppExeName "Ali app.exe"
#define MyIcon ".\assets\app.ico"

[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId={{16F28128-9DA7-4F41-BA68-7A2E4A6E1D4C}
AppName={#MyAppName}
...
; Ali requires Windows 10 22H2 or newer for proper unicode rendering
...
Source: ".\assets\open-ide.ps1"; DestDir: "{app}"; Flags: ignoreversion

[Icons]
Name: "{group}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\app.ico"
Name: "{group}\{#MyAppName} IDE"; Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\open-ide.ps1"""; IconFilename: "{app}\app.ico"
Name: "{app}\lib\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\app.ico"
Name: "{app}\lib\{#MyAppName} IDE"; Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\open-ide.ps1"""; IconFilename: "{app}\app.ico"
Name: "{userprograms}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\app.ico"
Name: "{userprograms}\{#MyAppName} IDE"; Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\open-ide.ps1"""; IconFilename: "{app}\app.ico"
...
SetupAppRunningError=Another Ali installer is running.%n%nPlease cancel or finish the other installer, then click OK to continue with this install, or Cancel to exit.
...
function URLDownloadToFileA(Caller: LongInt; URL: String; FileName: String; Reserved: LongInt; StatusCB: LongInt): HRESULT;
...
var
  hr: HRESULT;
begin
  Log('Downloading required runtime: ' + URL);
  hr := URLDownloadToFileA(0, URL, DestFile, 0, 0);
  Result := hr = 0;
end;

function DownloadVCRedist(const URL, DestFile: string): Boolean;
begin
  if FileExists(DestFile) then begin
    Result := True;
    exit;
  end;
  Result := DownloadFile(URL, DestFile);
  if not Result then begin
    MsgBox('Unable to download required Visual C++ runtime from the internet.' + #13#10 +
      'Please check your network connection and try again.' + #13#10 +
      URL, mbError, MB_OK);
  end;
end;
...
begin
  Result := True;
  if vc_redist_needed() then begin
    if IsArm64() then begin
      RuntimeURL := 'https://aka.ms/vs/17/release/vc_redist.arm64.exe';
      RuntimeFile := ExpandConstant('{tmp}\vc_redist.arm64.exe');
    end else begin
      RuntimeURL := 'https://aka.ms/vs/17/release/vc_redist.x64.exe';
      RuntimeFile := ExpandConstant('{tmp}\vc_redist.x64.exe');
    end;

    if not DownloadVCRedist(RuntimeURL, RuntimeFile) then begin
      Result := False;
      exit;
    end;

    if not FileExists(RuntimeFile) then begin
      MsgBox('Downloaded runtime installer was not found: ' + RuntimeFile + #13#10 +
        'Installation cannot continue.', mbError, MB_OK);
      Result := False;
      exit;
    end;
  end;
