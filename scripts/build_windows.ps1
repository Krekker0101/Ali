...
function checkEnv {
    ...
    if (Get-Command Import-AliVSDeveloperEnvironment -ErrorAction SilentlyContinue) {
        if (Import-AliVSDeveloperEnvironment) {
            Write-Output "Visual Studio C++ build environment initialized"
        } else {
            Write-Output "WARNING: Visual Studio C++ build environment was not initialized; CMake may fail if cl.exe, nmake.exe, or Windows SDK are missing"
        }
    }
    ...
    if ($env:INNO_SETUP_DIR) {
        $innoCandidates += $env:INNO_SETUP_DIR
    }
    $innoCandidates += @(
    ...
    foreach ($candidate in $innoCandidates) {
        if ($candidate -and (Test-Path -LiteralPath (Join-Path $candidate "ISCC.exe"))) {
            $script:INNO_SETUP_DIR=$candidate
            break
        }
    }
    if ($null -eq ${script:INNO_SETUP_DIR}) {
        $inoSetup=(get-item "C:\Program Files*\Inno Setup*\" -ErrorAction SilentlyContinue)
        if ($inoSetup.length -gt 0) {
            $script:INNO_SETUP_DIR=$inoSetup[0].FullName
        }
    }
    ...
    if (!$env:VERSION) {
        ...
        if (Test-Path -LiteralPath ".git") {
            $data=(git describe --tags --first-parent --abbrev=7 --long --dirty --always 2>$null)
        }
        $pattern="v(.+)"
    ...
    Write-Output "Building Ali $script:VERSION with package version $script:PKG_VERSION"
